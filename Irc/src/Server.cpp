...
/*   Updated: 2024/10/18 17:15:29 by larmogid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::handleCommand(const std::string& command, Client* client) {
    ...
    if (!client->isVerified()) {
        // Se il comando è "PASS", procedi con la verifica
        if (cmd == "PASS") {
        ...
        // Se non è stato ancora inviato un messaggio di richiesta password, invialo
        ...
        return; // Esci dalla funzione se il client non è ancora verificato
    } else if (cmd == "NICK") {
    // Estrai il nickname dal comando
    std::string nickname;
    iss >> nickname;

    // Verifica se il nickname è vuoto
    if (nickname.empty()) {
    ...
    // Imposta il nickname per il client
    ...
    client->authenticate(); 
    // Niente messaggio di benvenuto qui!
    } else if (!client->isNicknameSet()) {
        // Se il client non ha ancora impostato un nickname, invia un messaggio che lo richiede
        std::string invite_msg = "431 :No nickname given. Please choose a nickname to proceed.\n";
        
        // Invia il messaggio al client
        send(client->getFd(), invite_msg.c_str(), invite_msg.size(), 0);
        std::cout << "Server log: Sent nickname request to client FD " << client->getFd() << std::endl;

        return; // Esci dalla funzione se il nickname non è impostato
    }
    else if (cmd == "USER") {
        std::string username;
